...
export default function Home() {
  ...
  const handleChange = (e: any) => {
    ...
    const filteredData = dummyData.find(
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    const filteredDummyData = dummyData.find(
      (item) => item.name.toLowerCase() === searchItem.toLowerCase(),
    );
    setSearchResult(
      filteredDummyData ? filteredDummyData.name : "No results found",
    );
      ...
      <div className="flex flex-col items-center gap-4 gap-8">
        ...
        <div className="flex flex-row justify-center items-center gap-4">
          <input
            ...
            type="text"
            placeholder="Search"
            className="rounded-md border-2 border-gray-300 px-4 py-2 focus:border-blue-500 focus:outline-none dark:bg-gray-800 dark:border-gray-600 dark:focus:border-blue-400"
          />
          <button
            className="bg-green-300 border border-gray-300 px-4 py-2 rounded-md"
